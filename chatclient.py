...
#------------------------------------------------------------------------------
#   Connects to the server to log out 
#------------------------------------------------------------------------------

    def logout(self):    
        self.request['logout']['USER'] = self.username
        if self.connect_remote_server(self.request['logout']):
...
#------------------------------------------------------------------------------------------------
    ...
    def add_contact(self, results):
        ...
        self.request['add']['CONTACT'] = results['USER']
        results = self.connect_remote_server(self.request['add'])
        return results
...
#------------------------------------------------------------------------------
    ...
    def chat_button_clicked(self,name):  
        ...
        if self.chat_connection(name) == 1: # 1st connection - new tab
            ...
            for n in self.tab_name_user.keys():
...
#-----------------------------------------------------------------------------------------
    ...
    def chat_connection(self,name):
        ...
        if name not in self.user_connection.keys():            
            ...
            results = (self.connect_remote_server(self.request['query']))            
...
#------------------------------------------------------------------------------
#   Adds the message to the outgoing messages queue  
#------------------------------------------------------------------------------    
...
#------------------------------------------------------------------------------
#   Local erver for listening from other clients
#------------------------------------------------------------------------------

    def local_server(self):
        'Initialise the instance with an IP address and port number'
        self.host = '10.40.216.156' #host  
        self.port = 8080 #port        
